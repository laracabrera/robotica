...
El agente es el robot que juega al Go y el entorno es el tablero de Go.

Un estado del entorno es una colocacion concreta de las piedras.

Dado un estado, se pueden hacer un número finito de movimientos (acciones).

- Los movimientos (legales) están determinados por el entorno.
- Por ejemplo, no se puede poner una piedra en un lugar ocupado.

Cuando el agente realiza una acción (e.g. poner piedra):

- El entorno la recibe como entrada
- Genera un estado resultado y una recompensa
...
![](../img/t4/Modelo%20agente-entorno.svg)
</center>

El proceso de aprendizaje por refuerzo es el siguiente:

1. Nuestro Agente recibe el estado $s_0$ del entorno.
2. De acuerdo a ese estado $S_0$, el agente realiza la acción $A_0$.
3. El entorno pasa al nuevo estado $S_1$.
4. El entorno una recompensa o castigo $R_1$ al agente.
...
El **objetivo** del agente será **maximizar** la **recompensa acumulada**.

- A este valor se le denomina **rendimiento esperado** (<i>expected return</i>).
